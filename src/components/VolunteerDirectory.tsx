...
export function VolunteerDirectory({ volunteers }: VolunteerDirectoryProps) {
  const groupedByCity = volunteers.reduce((acc, volunteer) => {
    if (!acc[volunteer.city]) {
      acc[volunteer.city] = []
    }
    acc[volunteer.city].push(volunteer)
    return acc
    ...
    <div className="container mx-auto px-4 md:px-8 py-12">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="space-y-8">
          {Object.entries(groupedByCity).map(([city, cityVolunteers]) => (
            <div key={city}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {cityVolunteers.map((volunteer) => (
                  <Card key={volunteer.id} className="hover:shadow-lg transition-shadow">
                    ...
                    <CardContent>
                      <p className="text-sm text-muted-foreground mb-3">{volunteer.role}</p>
                      <Button variant="outline" size="sm" className="w-full" asChild>
                        <a href={`tel:${volunteer.mobile}`}>
