# Sindhi Matrimony "ShaadiPartnerSearch" - Product Requirements Document

A selfless community service platform connecting Sindhi families for matrimonial alliances, built on traditional values with modern technology.
...
2. **Welcoming** - The interface should feel warm and approachable, like being welcomed into a Sindhi home, with Hindi/Devanagari script creating cultural familiarity
...
- **Functionality**: Advanced search with filters for gender, age range, location, gotra/kul, education, and occupation
- **Purpose**: Help families find compatible matches within the Sindhi community based on traditional and modern criteria
...
- **Functionality**: Multi-step form capturing personal details, family background, education, profession, gotra, photos, and contact information
...
- **Progression**: Fill personal details → Add family & gotra info → Education & career → Upload photo → Review & submit → OTP verification → Awaiting manual approval → Profile goes live with verification badge
- **Success criteria**: Form validates all required fields, photo uploads successfully, OTP verification works, profile enters review queue, user receives confirmation
...
- **Functionality**: City-wise volunteer directory with contact details, volunteer dashboard for profile reviews, event coordination
- **Purpose**: Enable community-driven moderation, provide personalized assistance, organize offline meet-ups
- **Trigger**: User clicks "Community Support" or volunteer logs into admin panel
- **Progression**: View volunteer list by city → Contact volunteer → OR → Volunteer reviews pending profiles → Approve/reject/request info → Profile status updates
- **Success criteria**: Volunteer contacts are accessible, review workflow is clear, profile approval/rejection works smoothly

### Resources & Guidance Section
- **Functionality**: Educational content about Sindhi wedding traditions, gotra matching, relationship advice, safety guidelines
- **Purpose**: Educate users about cultural practices, set expectations, provide value beyond just matchmaking
- **Trigger**: User clicks "Resources" or specific article from home page
- **Progression**: Browse resource categories → Click article → Read content → Share with family → Return to search
- **Success criteria**: Articles load quickly, content is well-organized, clear typography for Hindi/English mixed content
...
- **Purpose**: Sustain the platform through community support, recruit volunteers, demonstrate non-profit nature
- **Trigger**: User clicks "Support Us" or donation CTA
- **Progression**: View volunteering options → Fill volunteer form → OR → View donation info → Make payment → Receive acknowledgment
- **Success criteria**: Volunteer form submits successfully, donation details are clear, users understand the non-profit mission
...
- **Language Barriers**: Bilingual interface (Hindi + English), simple language avoiding technical jargon
- **Age/Browser Compatibility**: Responsive design works on all devices, accessible for users with limited tech literacy

## Design Direction

The design should evoke **warmth, trust, and cultural pride** - imagine the feeling of a Sindhi family gathering where tradition meets modernity. The interface should feel welcoming yet dignified, colorful yet sophisticated, reflecting the vibrant Sindhi culture while maintaining the seriousness of matrimonial alliances. Users should feel they're in a safe, community-driven space, not a commercial platform.

## Color Selection

Drawing inspiration from traditional Sindhi textiles, weddings, and cultural aesthetics with warm, trustworthy tones:

- **Primary Color**: Deep Maroon `oklch(0.35 0.15 15)` - Represents tradition, commitment, and the sacred nature of marriage; evokes the rich colors of Sindhi bridal wear
- **Secondary Colors**: 
...
- **Accent Color**: Vibrant Saffron `oklch(0.70 0.18 50)` - Draws attention to important actions like "Register Profile" and "Search"; culturally significant
...
**Components**:
- **Card**: Profile cards, volunteer contact cards, resource article previews, stat displays
- **Dialog**: Profile detail view, contact request modals, verification instructions, terms acceptance
- **Form** + **Input** + **Label**: Registration forms, search filters, volunteer applications
- **Select**: Dropdown for location, education level, occupation, gotra selection
- **Button**: Primary (register, search), Secondary (view profile, learn more), Ghost (navigation)
- **Badge**: Verification status, trust level indicators, "new" profile markers
- **Tabs**: Switch between different sections (about me, family, preferences) in profile view
- **Alert**: Privacy notices, verification pending status, success confirmations
- **Avatar**: User profile pictures with fallback initials
- **Separator**: Divide form sections and content areas elegantly
- **Scroll Area**: Long lists of profiles, resource articles, volunteer directories
...
- Form inputs have Hindi placeholder text support with proper spacing
...
- Profile/User: User, UserCircle
- Verification: CheckCircle, ShieldCheck, Seal
- Location: MapPin
- Contact: Phone, Envelope
- Community: Users, UsersThree
- Resources: BookOpen, Article
- Privacy: Lock, Eye, EyeSlash
- Edit: PencilSimple
- Settings: Gear
